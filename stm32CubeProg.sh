...
case "${UNAME_OS}" in
  ...
  Darwin*)
    ...
    ;;
  Windows*)
    STM32CP_CLI=${BASEDIR}/win/STM32CubeProgrammer/bin/STM32_Programmer_CLI.exe
#    if ! command -v $STM32CP_CLI >/dev/null 2>&1; then
...
STM32TRIGGER="${BASEDIR}/win/stm32trigger/stm32trigger.exe"
...
case "${INTERFACE}" in
  ...
  serial)
    ...
    ${STM32TRIGGER} enter-to-bootloader ${PORT}
#    ${STM32CP_CLI} --connect port="${PORT}" "${RTS}" "${DTR}" "${ERASE}" --quietMode --download "${FILEPATH}" "${ADDRESS}" --start "${ADDRESS}"
    ${STM32CP_CLI} --connect port="${PORT}" "${RTS}" "${DTR}" "${ERASE}" --quietMode --download "${FILEPATH}" "${ADDRESS}"
    ${STM32TRIGGER} reset ${PORT}
    ;;
  *)
